...
* `cargo run -p swarmy-client --example tail` follows the durable log of each
  ...
  later scans. It rotates groups of up to 32 logs on one open stream, replaying from each log's last delivered cursor.
...
chosen by the caller. If the server rejects a change, `next` reports the
error once and the caller must set a valid subscription. A disconnected stream replays from its last delivered
