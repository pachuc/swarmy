//! Offline root filesystem construction for base images.
//!
//! This crate builds a fixed-size ext4 file from a recipe without touching
//! the network, the chunk store, or cluster metadata, so the client binary
//! can build an image locally and stream the bytes to the control plane for
//! publication.
use std::{
    fs::{self, File},
    io::Write,
...
impl Recipe {
    ...
    pub fn load(path: &Path) -> Result<(Self, PathBuf, String)> {
        ...
        check_disk_size(recipe.disk_size)?;
...
/// Disk sizes must hold whole chunks, matching the manifest layout.
/// # Errors
/// Rejects zero and non-multiple sizes.
fn check_disk_size(size: u64) -> Result<()> {
    if size == 0 || !size.is_multiple_of(u64::from(CHUNK_SIZE)) {
        return Err(ImageError::Invalid(
            "disk_size must be a positive multiple of the chunk size".into(),
        ));
    }
    Ok(())
}
